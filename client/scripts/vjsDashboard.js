...
function setupTabsAndButtons() {
	...
	$("#tabs").height(config.defaultApplicationHeight);
	calcDashboardContainerHeight();
...
function setupDragAndDrop() {
	...
	$('html').click(function() {
		if ($("#drag").isInsideOf(getCurrentDashboard())) {
			...
			addReport($("#drag").attr("report-uri"), $("#drag").attr("report-title"),
						...
						config.defaultReportWidth, config.defaultReportHeight);
	...
	$("#gallery").on("mousedown", ".addReportBtn", function(e) {
		var $newReportContainer = $("<div id='drag'></div>")
			.css({
				width: config.defaultReportWidth,
				height: config.defaultReportHeight, 
				position: "fixed",
				left: e.pageX,
				top: e.pageY
			})
...
function launchVisualizeJs() {
	// Log into JRS via visualize.js and create a session object
	// that will be used though out the application.
	visualize(
		{
			server: config.jrsUrl,
			auth: {
...
$(document).ready(function() {
	...
	// Load Visualize.js script from JRS
	$.getScript(config.jrsUrl + "/client/visualize.js", function() {
    	launchVisualizeJs();
	});
});
